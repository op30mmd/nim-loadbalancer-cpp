...
// Global logger instance (declared extern in logger.h). Defined here so both
// the server executable and the test suite link against a single instance.
Logger g_logger;

// ============================================================================
// Construction & Route Wiring
// ============================================================================

ProxyHandlers::ProxyHandlers(KeyManager& key_manager, ClientSideBackoff& backoff_manager,
	ModelCache& model_cache, ProviderManager& provider_manager, StatsCollector& stats)
	: key_manager(key_manager), backoff_manager(backoff_manager),
	model_cache(model_cache), provider_manager(provider_manager), stats(stats) {
}

void ProxyHandlers::setup_routes(httplib::Server& svr) {
	...
	svr.Get("/v1/keys", [this](const httplib::Request& req, httplib::Response& res) {
		handle_keys(req, res);
	});

	// --- ENDPOINT: GET /v1/models & GET /models ---
	auto models_route = [this](const httplib::Request& req, httplib::Response& res) {
		handle_models(req, res);
	};
	svr.Get("/v1/models", models_route);
	svr.Get("/models", models_route);

	// --- ANTHROPIC MESSAGES ENDPOINTS: POST /v1/messages & POST /messages ---
	svr.Post("/v1/messages", [this](const httplib::Request& req, httplib::Response& res) {
		handle_anthropic_messages(req, res);
	});
	svr.Post("/messages", [this](const httplib::Request& req, httplib::Response& res) {
		handle_anthropic_messages(req, res);
	});

	// --- GENERIC OPENAI WILDCARD ROUTE HANDLER ---
	auto openai_route = [this](const httplib::Request& req, httplib::Response& res) {
		handle_openai_proxy(req, res);
	};
	svr.Post("/v1/chat/completions", openai_route);
	svr.Post("/v1/completions", openai_route);
	svr.Post("/v1/embeddings", openai_route);
	svr.Post("/v1/chat", openai_route);
	svr.Post("/v1/edits", openai_route);
	svr.Post("/v1/images/generations", openai_route);
	svr.Post("/v1/audio/transcriptions", openai_route);
	svr.Post("/v1/audio/translations", openai_route);
	svr.Post("/v1/moderations", openai_route);
	svr.Post("/v1/rerank", openai_route);
	svr.Get("/v1/chat/completions", openai_route);

	// --- ENDPOINT: GET /v1/providers (provider status) ---
	// NOTE: must be registered BEFORE the wildcard routes below; httplib
	// matches routes in registration order, so the wildcard would otherwise
	// swallow it.
	svr.Get("/v1/providers", [this](const httplib::Request& req, httplib::Response& res) {
		handle_providers(req, res);
	});

	// Wildcard passthrough routes (last: they match everything else)
	svr.Post("/v1/(.*)", openai_route);
	svr.Get("/v1/(.*)", openai_route);
}
...
std::tuple<BackendProvider*, std::string, std::string> ProxyHandlers::get_next_backend() {
	auto [prov, key] = provider_manager.get_next_provider_and_key();
	if (prov && !key.empty()) {
		return { prov, key, prov->base_url };
	}
	// Legacy fallback
	std::string legacy_key = key_manager.get_key();
	return { nullptr, legacy_key, get_nvidia_base_url() };
}
...
void ProxyHandlers::handle_health(const httplib::Request&, httplib::Response& res) {
	nlohmann::json h = { {"status", "ok"}, {"service", "nim-proxy-cpp"} };
	res.status = 200;
	res.set_content(h.dump(), "application/json");
}

void ProxyHandlers::handle_keys(const httplib::Request&, httplib::Response& res) {
...
void ProxyHandlers::handle_models(const httplib::Request& req, httplib::Response& res) {
	...
	auto [prov, key, base_url] = get_next_backend();
	if (key.empty()) {
		LOG_ERROR("ModelsAPI", "No API keys / providers available.");
		res.status = 503;
		res.set_content("{\"error\": \"No API keys or providers available\"}", "application/json");
		return;
	}

	CURL* curl = curl_easy_init();
	if (!curl) {
		if (prov) provider_manager.mark_provider_failed(prov, key, 500);
		else key_manager.mark_failed(key, 500);
		res.status = 500;
	...
	if (curl_res != CURLE_OK) {
		LOG_WARN("ModelsAPI", "Transient network error: " + std::string(curl_easy_strerror(curl_res)));
		if (prov) provider_manager.mark_provider_failed(prov, key, -1);
		else key_manager.mark_failed(key, -1);
		res.status = 502;
		res.set_content("{\"error\": \"Upstream network error\"}", "application/json");
		return;
	}

	if (http_code == 429 || http_code == 401 || http_code == 403) {
		if (prov) provider_manager.mark_provider_failed(prov, key, http_code);
		else key_manager.mark_failed(key, http_code);
		res.status = http_code;
		res.set_content("{\"error\": \"Upstream authentication or rate limit\"}", "application/json");
		return;
	}

	if (http_code == 200) {
		if (prov) provider_manager.mark_provider_success(prov, key);
		else key_manager.mark_success(key);
...
void ProxyHandlers::handle_anthropic_messages(const httplib::Request& req, httplib::Response& res) {
	auto _req_start = std::chrono::steady_clock::now();
	auto _record_stats = [&]() {
		auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
			std::chrono::steady_clock::now() - _req_start).count();
		stats.record_request(res.status, elapsed, "/v1/messages", res.status == 200);
	};
	struct _StatsGuard {
	...
	auto [prov, key, base_url] = get_next_backend();
	if (key.empty()) {
		LOG_ERROR("AnthropicAPI", "No API keys or providers available.");
		res.status = 503;
		res.set_content(make_anthropic_error("overloaded_error", "No providers/keys available").dump(), "application/json");
		return;
	}

	std::string upstream_url = base_url + "/chat/completions";
	std::string masked_key = (key.size() > 6) ? key.substr(key.size() - 6) : key;
	LOG_DEBUG("AnthropicAPI", "Routing via " + (prov ? prov->name : "legacy") + " | Key ..." + masked_key);
	...
	if (!curl) {
		...
		res.status = 500;
		res.set_content(make_anthropic_error("api_error", "Internal error creating request").dump(), "application/json");
		return;
	}
	...
	if (ctx->curl_failed) {
		LOG_WARN("AnthropicAPI", "Upstream network transfer failed: " + ctx->curl_error_msg);
		...
		res.status = 502;
		res.set_content(make_anthropic_error("overloaded_error", "Upstream network error").dump(), "application/json");
		return;
	}
	...
	if (status_code == 429 || status_code == 401 || status_code == 403 || status_code == 502 || status_code == 503) {
		...
		if (prov) provider_manager.mark_provider_failed(prov, key, status_code);
		else key_manager.mark_failed(key, status_code);

		std::string err_type = (status_code == 429) ? "rate_limit_error" : "overloaded_error";
		res.status = status_code;
		res.set_content(make_anthropic_error(err_type, "Upstream error").dump(), "application/json");
		return;
	}
	...
	if (ctx->is_stream) {
		...
		lstate->on_destroy = [this]() { stats.stream_ended(); };

		res.set_chunked_content_provider(
			"text/event-stream",
			[lstate, this](size_t offset, httplib::DataSink& sink) {
				if (!sink.is_writable()) {
					LOG_WARN("Stream", "Downstream client aborted connection mid-stream.");
					return false;
				}
				if (streaming_shutdown.load()) return false;
...
void ProxyHandlers::handle_openai_proxy(const httplib::Request& req, httplib::Response& res) {
	auto _req_start = std::chrono::steady_clock::now();
	...
	auto _record_stats = [&]() {
		...
		auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
			std::chrono::steady_clock::now() - _req_start).count();
		stats.record_request(res.status, elapsed, _ep, res.status == 200);
	};
	struct _StatsGuard {
		std::function<void()> fn;
		~_StatsGuard() { fn(); }
	...
	auto [prov, key, base_url] = get_next_backend();
	if (key.empty()) {
		LOG_ERROR("OpenAIAPI", "No API keys or providers available.");
		res.status = 503;
		res.set_content("{\"error\": \"No providers/keys available\"}", "application/json");
		return;
	}

	std::string upstream_url = base_url + "/" + path;
	std::string masked_key = (key.size() > 6) ? key.substr(key.size() - 6) : key;
	LOG_DEBUG("OpenAIAPI", "Routing via " + (prov ? prov->name : "legacy") + " | Key ..." + masked_key);
	...
	if (ctx->curl_failed) {
		LOG_WARN("OpenAIAPI", "Upstream network transfer failed: " + ctx->curl_error_msg);
		if (curl_thread->joinable()) curl_thread->join();
		curl_easy_cleanup(curl);
		curl_slist_free_all(headers_list);
...
void ProxyHandlers::handle_providers(const httplib::Request&, httplib::Response& res) {
	...
	res.status = 200;
	res.set_content(nlohmann::json({{"providers", arr}}).dump(), "application/json");
}
