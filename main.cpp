...
int main() {
	...
	ProviderManager g_provider_manager;
	g_provider_manager.initialize_default(keys, key_cooldown, key_max_cooldown);

	// Helper to get next provider + key (prefers configured providers, falls back to legacy KeyManager)
	auto get_next_backend = [&]() -> std::tuple<BackendProvider*, std::string, std::string> {
		auto [prov, key] = g_provider_manager.get_next_provider_and_key();
		if (prov && !key.empty()) {
			return {prov, key, prov->base_url};
		}
		// Legacy fallback
		std::string legacy_key = key_manager.get_key();
		return {nullptr, legacy_key, NVIDIA_BASE_URL};
	};
	...
	auto models_handler = [&](const httplib::Request& req, httplib::Response& res) {
		...
		if (!curl) {
			if (prov) g_provider_manager.mark_provider_failed(prov, key, 500);
		...
		if (curl_res != CURLE_OK) {
			...
			if (prov) g_provider_manager.mark_provider_failed(prov, key, -1);
		...
		if (http_code == 429 || http_code == 401 || http_code == 403) {
			if (prov) g_provider_manager.mark_provider_failed(prov, key, http_code);
		...
		if (http_code == 200) {
			if (prov) g_provider_manager.mark_provider_success(prov, key);
	...
	auto handle_anthropic_messages = [&](const httplib::Request& req, httplib::Response& res) {
		...
		if (status_code == 429 || status_code == 401 || status_code == 403 || status_code == 502 || status_code == 503) {
			...
			if (prov) g_provider_manager.mark_provider_failed(prov, key, status_code);
	...
	// Add a new endpoint to expose provider status
	svr.Get("/v1/providers", [&](const httplib::Request&, httplib::Response& res) {
		auto snap = g_provider_manager.snapshot();
		nlohmann::json arr = nlohmann::json::array();
		for (const auto& s : snap) {
			arr.push_back({
				{"name", s.name},
